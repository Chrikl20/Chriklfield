...
2. In einem neuen Supabase-Projekt **alle fünf Migrationen in Dateinamenreihenfolge** anwenden. Im Projekt `tslgkbtfdchkfriuealv` ist dies am 9. September bereits erledigt. Dort nichts erneut einfügen oder zurücksetzen. Die früheren vier Dateinamen wurden mit der tatsächlichen Supabase-Historie abgeglichen; Details für bestehende andere Datenbanken: [supabase-setup.md](supabase-setup.md). Der Publishable Key erlaubt keine Schema-Migrationen.
3. Trigger.dev-Projekt einrichten und `npm run worker:deploy` mit dem eigenen autorisierten Konto ausführen. Die Tasks `media-upload` und `recover-uploads` müssen aktiv sein, zusätzlich zu den vier bisherigen Tasks. Backendvariablen in Trigger setzen; der Upload-Worker benötigt Supabase-URL und Backend-Schlüssel. Er benötigt weder fal noch Stripe für reine Datei-Uploads. Die FFmpeg-Erweiterung ist bereits konfiguriert.
...
| `SUPABASE_SERVICE_ROLE_KEY`            | Geheimer Backend-Schlüssel des gleichen Projekts                                 | Ja        | Ja                                |
