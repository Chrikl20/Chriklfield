...
export async function createQuote(actor: Actor, input: JobInput): Promise<Quote> {
  ...
  if (input.characterId) {
    ...
    if (input.model === 'image') {
      ...
      ) as (CharacterVersion & { provider_reference_id?: string | null }) | null;

      requireCondition(v?.provider_reference_id, 'TRAINING_REQUIRED');
      input = { ...input, versionId: v.id };
      context.providerReferenceId = v.provider_reference_id;
